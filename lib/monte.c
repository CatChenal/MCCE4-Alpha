    ...
    for (i=0; i<env.titr_steps; i++) {
        ...
        */
        ...
        if (env.ms_out && new_ms_flag){
            // fname matches titr point float type:
            sprintf(sbuff, "%s/pH%.2feH%.2fms.txt", MS_DIR, ph, eh);
...
int print_mfe(int i_res, float mfeP, FILE *pK_fp, FILE *res_fp)
{
    ...
    if (abs(i_high-i_low) == 0) { // do mfe at only one point
        get_mfe(i_res, i_low);
        if (env.titr_type == 'p')
            fprintf(pK_fp, "  %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %10.2f\n",
        ...
        for (j=0; j<n_all; j++) {
            ...
            if (env.titr_type == 'p')
                fprintf(res_fp, "%s  %s   %9.2f %9.2f %9.2f %9.2f\n", head1, head2,

...
from mcce4.mcce_ftpl_agent.tools.mcce_tools import (convert_cif_to_pdb,
...
EXAMPLES:
    ...
    * With an input file (.pdb or .cif):
    ...
    %(prog)s EMH.pdb --gui                                 # Web GUI, requires an input file (& Streamlit)
"""
...
ASSIGNED_LIGID = "UNL"  # when lig-smiles provided w/o lig-id
...
Optional if lig-id is provided; default: %(default)s"""
                        )
    parser.add_argument("-lig-id",
                        type=str,
                        default=None,
                        help="""3-letter RCSB ligand code (e.g., EMH). If provided without
lig-smiles, SMILES and metadata are fetched from RCSB; default: %(default)s"""
                        )
    parser.add_argument("-lig-smiles",
                        type=str,
                        nargs="?",
                        default=None,
                        help="""The ligand SMILES string. If provided without lig-id, lig-id
is set to 'UNL'. The 3D structure is built from SMILES using RDKit; default: %(default)s"""
                        )
...
from mean pKa to consider; default: %(default)s"""
    ...
    dimorphite_group.add_argument("-max-variants",
                                  ...
                                  help="Max protonation variants per compound; default: %(default)s"
                                  )
    dimorphite_group.add_argument("--label-states",
                                  default=DIMORPHITE_LABEL_STATES,
                                  action="store_true",
                                  help="Label output states SMILES; default: %(default)s"
                                  )
...
def validate_args(args: argparse.Namespace):
    """
    Do file check & options combos validations
    """
    # gui w/o file:
    ...
    # nothing provided:
    if all([args.input_file is None, args.lig_id is None, args.lig_smiles is None]):
        sys.exit("ERROR: At least one of: an input PDB/CIF file, -lig-id or -lig-id and -lig-smiles must be provided.")
    
    if args.lig_smiles is not None:
        if args.lig_id is None:
            args.lig_id = ASSIGNED_LIGID
            print("Missing lig-id set to", ASSIGNED_LIGID)
        else:
            args.lig_id = args.lig_id.upper()

    # pdb and smiles:  error
    if all([args.input_file is not None, args.lig_smiles is not None]):
        sys.exit("ERROR bad combo: Choose either input_file [-lig-id] or -lig-id and -lig-smiles.")
    ...
    if args.input_file is not None:
        ...
        if not check_pdb_has_het(args.input_file):
           print("WARNING: Input file has no HETATM lines.")
...
def main(args: Union[argparse.Namespace, dict]):
    ...
    lig_id = "" if args.lig_id is None else args.lig_id.upper()
    pdb_path = None
    if args.input_file is not None:
        ...
        # Determine lig_id else extract from PDB
        if not lig_id:
    ...
    if pdb_path:
        ...
        logging.info(f"  Input: -lig-id {lig_id} (no PDB file: will build from RCSB SMILES)")
        if args.lig_smiles:

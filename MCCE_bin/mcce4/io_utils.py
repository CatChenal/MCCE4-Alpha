 ...
 * `textfile2df`: Loads most output files into a pandas.DataFrame without any processing.
...
def show_elapsed_time(start_t: time, writer: Callable = print, info: str = None):
    """If specific info is given, it is prepended to ' - Elapsed time:'.
    Argument 'writer' is an output function, e.g. print, logger.info, etc.
    """
    elapsed = time.time() - start_t
    if info is None:
        writer(f"Elapsed time: {elapsed:,.2f} s ({elapsed/60:,.2f} min).")
    else:
        writer(f"{info} - Elapsed time: {elapsed:,.2f} s ({elapsed/60:,.2f} min).")
    return
...
def config_logger(step_num: int, log_level: str = "INFO"):
    ...
    run_fh.setFormatter(logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s"))
...
class MsgFmt:
    """class to preclude eagerly exec of f-strings."""
...
class ToDf:
    ...
    def reset_pk_oob(row):
        """Arg row is from a pd.df loaded from pK.out, after addition of the 'note' column."""
        # print("type(row.loc['pKa/Em']):", type(row.loc["pKa/Em"]), "\n")
    ...
    def __call__(self, fp: Path, file_type: str = None) -> Union[pd.DataFrame, None]:
        ...
        if which not in DF_READY_FILES:
            logger.error(
                mf("File {!r} cannot be loaded into a pandas.DataFrame.", which)
            )
            return None
        ...
        if fname not in ToDf.headers_dict:
            ...
            if (fname in ["pK.out", "sum_crg.out"]) or (which in ["pK.out", "sum_crg.out"]):
                # replace -0. -> 0.
                try:
                    df.update(df.select_dtypes(include="float64").apply(lambda x: x + 0.0))
...
def textfile2df(fp: Path) -> pd.DataFrame:
    ...
    return pd.read_csv(fp, sep="\s+", converters=convert)  # noqa: W605
...
def files_diff(
    ...
    Args:
      ...
      out_name (str, None): User defined output filename or 'diff_' + file1.name if None.
    ...
    if file_type is None:
        ...
        if not diffing_ready(fp1, fp2):
            logger.error(f"Diffing not supported; files not in {DIFFING_FILES}.")
            return
    else:
    ...
    ph = ""
    # to get matched_colname from column index:
    matchcol = 0
    ...
    if which_name == "head3.lst":
        ...
        diff_df["note"] = [
            f"{n1}|{n2}" if ((n1 != "-") or (n2 != "-")) else pd.NA for (n1, n2) in zip(df1["note"], df2["note"])
        ]

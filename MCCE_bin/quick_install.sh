...
echo -e "Repo bin folders:\n $REPO_bin\n $MCCE_bin"

echo ""
echo "Getting NextGenPB.sif with $DOWNLOAD_CMD..."
...
then
  ...
  if [[ "$DOWNLOAD_CMD" == "curl" ]]; then
    ...
    echo "NGPB image file already exists."
...
then 
  ...
  then
    echo "'MCCE4/bin' already in PATH."
  fi
  if grep -q "$export2" "$rc_file";
  then
    echo "'MCCE4/MCCE_bin' already in PATH."
  fi
fi

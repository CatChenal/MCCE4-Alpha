    ...
    def sum_crg(self):
        ...
        for res in self.residues:
            ...
            for conf in res.conformers:
                ...
                total_crg = [
                    total_crg[i] + conf.crg * conf.occ[i] for i in range(npoints)
                ]
                total_proton_crg = [
                    total_proton_crg[i] + conf.nh * conf.occ[i] for i in range(npoints)
                ]
                total_redox_crg = [
                    total_redox_crg[i] - conf.ne * conf.occ[i] for i in range(npoints)
                ]
                total_ion_crg = [
                    total_ion_crg[i] + (conf.crg - conf.nh + conf.ne) * conf.occ[i]
        ...
        headline = " %3s       %s\n" % (
            self.titration_type,
            " ".join(["%5.2f" % x for x in self.titration_points]),
        )
    ...
    def fitpka(self):
        ...
        for res in self.residues:
            if res.state_flag != "0":
                ...
                try:
                    (popt, pcov) = curve_fit(
                        sigmoid, xdata, ydata, bounds=(0, [npoints, 4])
                    )
                ...
                # print(res_str, popt)
                if (
                    popt[0] < 0.001 or popt[0] > npoints - 1.001
                ):  # x from 0 to 14 as 15 points
                    msg = "Titration out of range"
                ...
                if msg:
                    ...
                    if titration_type.upper() == "PH":
                        ...
                        titration_type.upper() == "CH"
                        or titration_type.upper() == "EXTRA"
                    ):
                        nslope = 0.4342 * popt[1] / titration_delta * PH2KCAL
                    else:
                        print("Why am I here?")
                    pka_str = "%9.3f %9.3f %9.3f" % (
                        midpoint,
                        nslope,
                        chi_squared * 1000.0,
                    )
    ...
    def load_confs(self):
        ...
        while fort38_lines and head3_lines:
            ...
            if conf.name != fields[1]:
                print(
                    "Confomer %s in fort.38 does not match %s in head3.lst"
                    % (conf.name, fields[1])
                )
                sys.exit()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        help="""
User defined rule file. File format example:

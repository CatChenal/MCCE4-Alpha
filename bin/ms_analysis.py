...
def example():
    """Example function showing basic information when run in a mcce output
    dir for 4lzt (likely).
    Note: It will fail if protein does not contain "GLU-1A0035".
    """
    ...
    ms_in_bands = groupms_byenergy(microstates, ticks)
    print([len(band) for band in ms_in_bands])

    neutral, charged = groupms_byiconf(microstates, [12, 13, 14, 15])
    l_E, a_E, h_E = ms_energy_stat(microstates)
    print(l_E, a_E, h_E)
    ...
    diff_occ = whatchanged_conf(neutral, charged)
    for key in diff_occ.keys():
        print("%3d, %s: %6.3f" % (key, conformers[key].confid, diff_occ[key]))

    diff_bhd = whatchanged_res(neutral, charged, msout.free_residues)
    for i, ics in enumerate(msout.free_residues):
        print("%s: %6.4f" % (conformers[ics[0]].resid, diff_bhd[i]))

    charges = ms_convert2sumcrg(microstates, msout.free_residues)
    for i, ics in enumerate(msout.free_residues):
        print("%s: %6.4f" % (conformers[ics[0]].resid, charges[i]))

...
def sanitycheck_step1(t0):
    """
    Sanity check for step 1. 
    
    Iuput t0: start time of step 1 run

    Check if these files are generated and the time stamps are updated.

    Returns:
        STATUS: messages
        STATUS = SUCCESS, WARNNING, ERROR
    """
    status = "SUCCESS"
    messages = []
    ...
    for fname in expected_files:
        ...
        elif fname == "param/mcce.tpl" and Path(fname).exists():
            file_size = os.path.getsize(fname) / 1024  # Convert to KB
            min_fsize = 200 # Minimum expected size in kB
            if file_size < min_fsize:
                status = "WARNING"
                messages.append(f" {fname} size is {file_size:.1f} kB (expected >= {min_fsize} kB unless you use a custom ftpl folder)")

    # Final report
    if status == "SUCCESS":
        messages.append("Step 1 completed successfully.")
    else:

...
Output files:
Plots:
  Histogram figure: enthalpy_dist.pdf
...
def plot_hist_by_ms_energy(ms_by_enrg: list, save_dir:Path):
    ...
    fig_fp = save_dir.joinpath("enthalpy_dist.pdf")
    fig.savefig(fig_fp,
...
def rename_order_residues(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Return the amended dataframe:
        ...
        Reorder columns by acid first, then polar, base residues, all others in columns.
    """
    rename_dict = {}
    acid_list = []
    base_list = []
...
def get_topN_crg_microstates(mcce_dir: Path,
    ...
    msg = (f"Number of conformers: {len(conformers):,}\n",
